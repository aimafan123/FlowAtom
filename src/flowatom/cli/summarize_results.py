...
def main(argv=None) -> int:
    ...
    if len(set(paths)) != len(paths):
        parser.error("the same metrics file was supplied more than once")
    declared_seeds = [json.loads(path.read_text()).get("seed") for path in paths]
    known = [seed for seed in declared_seeds if seed is not None]
    if len(set(known)) != len(known):
        parser.error("duplicate declared seeds; aggregate one run per seed and setting")
    values = [_load(path, args.group) for path in paths]
    summary = {
        "group": args.group,
        "runs": [str(path) for path in paths],
        "values": values,
        "mean_micro_f1": float(np.mean(values)),
        "std_micro_f1": float(np.std(values, ddof=1)) if len(values) > 1 else None,
        "min_micro_f1": float(np.min(values)),
        "max_micro_f1": float(np.max(values)),
        "seeds": len(values),  # Historical field: number of supplied runs.
